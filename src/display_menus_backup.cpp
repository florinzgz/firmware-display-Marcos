...
void Display::showHiddenMenu() {
    ...
    for (int i = 0; i < 6; i++) {
        if (drawButton(x, y + i * spacing, buttonWidth, buttonHeight, options[i])) {
            // Debounce usando millis() en lugar de delay()
            static uint32_t lastMenuClickMs = 0;
            if (millis() - lastMenuClickMs >= 200) {
                lastMenuClickMs = millis();
                switch (i) {
...
void Display::showQuickMenu() {
    ...
    // Toggle Luces (debounce con millis())
    static uint32_t lastLightsToggleMs = 0;
    if (drawButton(panelX + 10, btnY, btnWidth, btnHeight, "Luces")) {
        if (millis() - lastLightsToggleMs >= 200) {
            lastLightsToggleMs = millis();
            vehicleData.status.lights = !vehicleData.status.lights;
        }
    }
    
    // Toggle 4x4 (debounce con millis())
    btnY += btnSpacing;
    static uint32_t last4x4ToggleMs = 0;
    if (drawButton(panelX + 10, btnY, btnWidth, btnHeight, "4x4")) {
        if (millis() - last4x4ToggleMs >= 200) {
            last4x4ToggleMs = millis();
    ...
    // Botón cerrar (debounce con millis())
    btnY = screenHeight - 60;
    static uint32_t lastCloseMs = 0;
    if (drawButton(panelX + 10, btnY, btnWidth, btnHeight, "Cerrar")) {
        if (millis() - lastCloseMs >= 200) {
            lastCloseMs = millis();
 ...
 */
// Variables estáticas para calibración no bloqueante
static uint8_t calibrationProgress = 0;
static uint32_t lastCalibProgressMs = 0;
static int calibrationType = -1;  // -1 = no calibrando
static bool calibrationComplete = false;
static uint32_t calibCompleteTimeMs = 0;

void Display::showCalibrationMenu() {
    // Si estamos en proceso de calibración, mostrar progreso
    if (calibrationType >= 0 && calibrationType < 4) {
        const char* items[] = {
            "Calibrar Pedal",
            "Calibrar Encoder",
            "Calibrar INA226",
        ...
        // Actualizar progreso cada 200ms (eliminado delay)
        if (millis() - lastCalibProgressMs >= 200) {
            ...
            if (calibrationProgress > 100) {
                ...
                calibCompleteTimeMs = millis();
            }
        }
        
        drawProgressBar(90, 150, 300, 40, calibrationProgress, items[calibrationType]);
        
        // Mostrar toast de completado por 2s (sin delay)
        if (calibrationComplete) {
            showToast("Calibracion completada", 2000);
            
            // Esperar 2s y volver (sin delay)
            if (millis() - calibCompleteTimeMs >= 2000) {
                calibrationType = -1;
        ...
        if (drawButton(x, y + i * spacing, buttonWidth, buttonHeight, items[i])) {
            ...
            if (millis() - lastButtonPressMs >= 200) {
                ...
                if (i == 4) {
                    ...
                    // Iniciar calibración (sin bloquear)
                    calibrationType = i;
                    calibrationProgress = 0;
                    calibrationComplete = false;
                    lastCalibProgressMs = millis();
